...
get_mfa_token(){
    if [ ! "$MFA_TOKEN" ]; then
        printf "Please enter your multifactor token code (press enter to skip): "
        read -r MFA_TOKEN
    else
        MFA_TOKEN="$*"
    fi
...
legacy_test(){
    [[ $1 =~ ^[0-9]*$ ]] && \
        main -s "$1" -u "$2" -d "$3" -r "$4" -m

        # Usage: source assume_role.sh {sourceAccountNumber} {username} {destinationAccountNumber} {rolename}
        # Enter MFA token code:
        # You have assumed the bastionCrossAccountAdmin role successfully.
}
...
prompt_args(){
    ...
    printf "Source Account (press enter to skip): "
    read -r AWS_ACCOUNT_NUMBER
    printf "Destination Account: "
    read -r DESTINATION_ACCOUNT
    printf "IAM User Name (press enter to skip): "
    read -r AWS_USER
    printf "Role: "
    read -r ROLE_NAME
    printf "Timeout (Default is 1h): "
    read -r TIMEOUT
    main -s "$AWS_ACCOUNT_NUMBER" -u "$AWS_USER" -d "$DESTINATION_ACCOUNT" -r "$ROLE_NAME" -t "$TIMEOUT" -m
}
...
print_help(){
    ...
    -m|--mfa            multi-factor (2fa/mfa) authentication (press enter to skip)
